...
export default function LoadChangeInstruction() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      console.log(result, 'result')
      if (result?.list?.length > 0) {
        const consumerData = result.list[0];

const current_month_outstandin_amt =
  Number(consumerData.netBill || 0) -
  Number(consumerData.paidAmt || 0);

const transformedData = transformDataKeys(consumerData);
...
if (transformedData.connection_type !== "Permanent") {
  setError({
    Consumer_id:
      "Load Change is allowed only for Permanent Connection"
  });
...
transformedData.current_month_outstandin_amt =
  current_month_outstandin_amt;

transformedData.current_bill_units = consumerData.billedUnits
  ? Math.round(consumerData.billedUnits)
  : 0;


// ------------------------------------------
// Load Enhancement Eligibility
// ------------------------------------------

const today = new Date();
today.setHours(0, 0, 0, 0);

let isDueDatePending = false;

if (consumerData.currentMonthDueDate) {

  // API format: DD/MM/YYYY
  const [day, month, year] =
    consumerData.currentMonthDueDate
      .split("/")
      .map(Number);
  ...
  // Due date abhi future mein hai
  // ya aaj due date hai
  isDueDatePending = today <= dueDate;
}
...
console.log(
  "Current Month Bill:",
  consumerData.currentMonthBill
);


// ------------------------------------------
// FINAL LOAD ENHANCEMENT CONDITION
// ------------------------------------------

if (
  Number(current_month_outstandin_amt) <=
    Number(consumerData.currentMonthBill || 0)
  ||
  isDueDatePending
) {

  console.log(
    "✅ Load Enhancement Allowed"
  );
