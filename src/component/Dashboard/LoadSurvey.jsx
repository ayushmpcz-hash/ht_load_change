...
//     const currentApplicationNo = items?.application_no; // Get current application number
//     try {
...
const LoadSurvey = () => {
  ...
  const survey_response = watch('survey_response');
  const existingMeCtRatio = items?.me_ct_ratio;
  ...
  const fetchEstimateDetails = async (erpField, type) => {
    console.log(type, 'type')
    const currentApplicationNo = items?.application_no;
    try {
      let erp_no = getValues(erpField);
      if (!erp_no) {
        setError(erpField, { type: 'manual', message: 'ERP Number is required' });
      ...
      // ✅ VALIDATION: Check if LONG_NAME contains the current application number
      const longName = projectData.LONG_NAME;
      const isApplicationNoPresent = longName && longName.includes(currentApplicationNo);
      ...
      // ✅ ME METER - NOW ACCEPTS ALL SCHEMECODES (DEPOSITE, SCCW, etc.)
      if (type === 'ME') {
        setIsMeErpFetched(true);
        setMeExtimateblock(true);
      }
      ...
      if (STATUS === 'Approved') {
        ...
        if (type === 'ME') {
          // ✅ ME METER - NO SCHEMECODE RESTRICTION
          setMeExtimateblock(true);
          setIsMeErpFetched(true);
          setValue('ndf_approved_by_name', APPROVED_BY_NAME);
          setValue('ndf_estimate_date', estimateDate);
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const employeeId = officerData?.employee_detail?.employee_login_id;
      console.log(employeeId, 'employeeId from survey')
      if (employeeId) {
        formData.append("employee_id", employeeId);
      }
