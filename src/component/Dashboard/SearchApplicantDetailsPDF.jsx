...
// ⭐ Ordered status sequence (1 to 12)
const STATUS_SEQUENCE = [
  {
    key: "registration_fee",
    label: "Pending for Registration Fee",
    matches: ["registration fee"],
  },
  {
    key: "resubmission",
    label: "Pending for Application Resubmission",
    matches: ["application resubmission", "resubmission"],
  },
  {
    key: "load_sanction",
    label: "Pending for Load Sanction",
    matches: ["load sanction"],
  },
  {
    key: "survey",
    label: "Pending for Survey",
    matches: ["survey"],
  },
  {
    key: "demand_note_generation",
    label: "Pending for Demand Note Generation",
    matches: ["demand note generation"],
  },
  {
    key: "demand_note_payment",
    label: "Pending for Demand Note Payment (Applicant Side)",
    matches: ["demand note payment"],
  },
  {
    key: "agreement_finalization",
    label: "Pending for Agreement Finalization",
    matches: ["agreement finalization"],
  },
  {
    key: "me_meter_issue",
    label: "Pending for ME Meter Issue",
    matches: ["me meter issue", "meter issue"],
  },
  {
    key: "upload_charging_permission",
    label: "Pending for Upload Charging Permission",
    matches: ["upload charging permission", "charging permission"],
  },
  {
    key: "commission_permission",
    label: "Pending for Commission Permission",
    matches: ["commission permission"],
  },
  {
    key: "commissioning",
    label: "Pending for Commissioning",
    matches: ["commissioning"],
  },
  {
    key: "connection_serve",
    label: "Connection Served",
    matches: ["connection serve", "connection served"],
  },
  {
    key: "cancelled",
    label: "Cancelled Application",
    matches: ["cancelled", "canceled"],
  },
];
...
const DownloadAllPdf = () => {
  ...
  // const DOCUMENT_BASE_URL = "https://htsanyojan.mpcz.in:8089";
   const DOCUMENT_BASE_URL = "https://htsanyojanuat.mpcz.in:8088";
  const token = Cookies.get("accessToken");

  // ⭐ Ordered stage summary with per-status applications for tooltip
  const stageSummary = useMemo(() => {
    const matchedApps = new Set(); // track already-matched app objects

    const orderedSummary = STATUS_SEQUENCE.map(({ key, label, matches }) => {
      const appsForStatus = applications.filter((app) => {
        if (matchedApps.has(app)) return false;
        const status = (app.application_status_text || "").toLowerCase();
        return matches.some((m) => status.includes(m));
      });

      appsForStatus.forEach((app) => matchedApps.add(app));

      return {
        key,
        label,
        count: appsForStatus.length,
        applications: appsForStatus,
      };
    });

    // Any leftover statuses not covered by the sequence → show at the end
    const unmatchedApps = applications.filter((app) => !matchedApps.has(app));
    const otherGroups = {};
    unmatchedApps.forEach((app) => {
      const st = app.application_status_text || "Status not available";
      if (!otherGroups[st]) {
        otherGroups[st] = {
          key: `other_${st}`,
          label: st,
      ...
      otherGroups[st].count += 1;
      otherGroups[st].applications.push(app);
    });

    return [...orderedSummary, ...Object.values(otherGroups)];
  }, [applications]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* ⭐ Application Summary - ordered sequence with hover tooltip */}
        <section
          className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm"
          aria-label="Application stage summary"
        >
          <div className="mb-3 flex items-baseline justify-between gap-3">
            <h3 className="text-base font-semibold text-[#0c0d52]">Application summary</h3>
            <span className="text-sm text-gray-600">
              Total received: <strong>{applications.length}</strong>
            </span>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stageSummary.map((item) => (
                <div key={item.key} className="relative group">
                  {/* Chip */}
                  <div
                    className={`rounded-md px-3 py-2 text-sm ring-1 transition-colors cursor-default ${
                      item.count > 0
                        ? "bg-gray-50 text-gray-700 ring-gray-200 hover:bg-blue-50 hover:ring-blue-300"
                  ...
                  >
                    ...
                    <strong
                      className={`ml-2 ${
                        item.count > 0 ? "text-[#0c0d52]" : "text-gray-400"
                      }`}
                    >
                      {item.count}
                    </strong>
                  ...
                  {/* Tooltip on hover (only when count > 0) */}
                  {item.count > 0 && (
                    <div
                      className="opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 absolute z-40 top-full left-0 bg-gray-900 text-white text-xs rounded shadow-xl min-w-[280px] max-w-[420px]"
                    >
                      <div className="px-3 py-2 font-semibold border-b border-gray-700 bg-gray-800 rounded-t">
                        {item.label} ({item.count})
                      </div>
                      <div className="px-3 py-2 max-h-[220px] overflow-y-auto space-y-1">
