...
const LoadSanction = () => {
 ...
 useEffect(() => {
  // Fetch CT/PT data only if:
  // - voltage is NOT high (<=33kV), OR
  // - voltage IS high but either transco_approval or edcra_approval exists
  if (HIGH_VOLT && !items?.transco_approval && !items?.edcra_approval) {
    return; // skip: still in EDCRA‑only stage
  }
