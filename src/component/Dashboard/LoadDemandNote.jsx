...
// 🔥 ME Amount Logic
const isMeSCCW = items?.survey?.ndf_scheme_name === "SCCW";

const meAmount = isMeSCCW
  ? (Number(items?.survey?.ndf_supervision_cost) || 0) +
    (Number(items?.survey?.ndf_supervision_cgst) || 0) +
    (Number(items?.survey?.ndf_supervision_sgst) || 0)
  : (Number(items?.survey?.ndf_total_amt) || 0);
...
const total_estimate_amt = extensionAmount + meAmount;
  ...
  let total_Amount_convert = Math.round(meAmount)
  // console.log(total_Amount_convert,'amounttttttttttt')
  return (
    <>
      <div>
